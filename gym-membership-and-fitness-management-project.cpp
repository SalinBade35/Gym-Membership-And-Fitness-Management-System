...
//	}
    ...
    {
    	system("cls");
    	fp.open("hamro1.bin",ios::out|ios::app);
    	m1.create_mem();
	...
	{	char ch='T';
    	...
    	cin>>p;
		fp.open("hamro1.bin",ios::in|ios::out);
    ...
    void delete_member(int num = 0)
    {
    	...
    	fstream fp2;
    	fp2.open("timro1.bin",ios::out|ios::binary);
    	fp.seekg(0,ios::beg);
    	while(fp.read((char*)&m1,sizeof(m1)))
    	{
    		if(m1.getmem()!=num)
...
//    	system("cls");
//    	num=n;
//    	fp.open("hamro1.bin", ios::app | ios::in | ios::binary);
//        fp.seekg(0,ios::beg);
//    	while(fp.read((char*)&m1,sizeof(m1)) && ch == 'T')
//    	{
...
void edit_member() {
    ...
    system("cls");
    num = n;
    fp.open("hamro1.bin", ios::in | ios::out | ios::binary);
    ...
    bool found = false;
    while (fp.read((char*)&m1, sizeof(m1))) {
        if (m1.getmem() == num) { // Enter only if user's input number matches
            m1.show_mem();
            cout << "\nPlease Enter The New details of the member: " << endl;
            m1.create_mem();
            int pos = static_cast<int>(fp.tellg()) - sizeof(m1);
...
{		
		...
		a:
		system("cls");
		cout<<"\t\t\t\tGYM MEMBERSHIP AND FITNESS MANAGEMENT SYSTEM\n"<<endl;
        cout<<"\t\t\t\t\t\tWELCOME\n\t\t\t\t";
        cout<<"*******************************************";
		cout<<endl<<"\n\n\t\tSELECT MODE"<<endl<<"\t\t1.User"<<endl<<"\t\t2.Admin"<<endl<<"\t\t3.Exit\n\n\t\t";
		cin>>i;
			if(i==1)
            {
            	system("cls");
